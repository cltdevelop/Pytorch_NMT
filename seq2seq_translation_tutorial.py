...
def readLangs(lang1, lang2, reverse=False):
    ...
    # 将每一行分成两列并进行标准化
    pairs = [[normalizeString(s) for s in l.split('\t')] for l in lines]  # [['go .', 'va !'], []]
...
注:
 ...
 :class:`optim.SGD` (`CUDA` and `CPU`),
 :class:`optim.SparseAdam` (`CUDA` and `CPU`)
 :class:`optim.Adagrad` (`CPU`)
...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        :param hidden_size: the size of each embedding vector
        """
...
#   attention" 在 `基于注意力的神经机器翻译的有效途径 <https://arxiv.org/abs/1508.04025>`__.
#
# 训练
# ========
#
